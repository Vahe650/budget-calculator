    ...
    recalculateAll(): void {
        ...
        const accumulate = (node: Category) => {
            if (node.type === 'revenue') {
                janRev += node.jan; febRev += node.feb; marRev += node.mar;
                aprRev += node.apr; mayRev += node.may; junRev += node.jun;
                julRev += node.jul; augRev += node.aug; sepRev += node.sep;
                octRev += node.oct; novRev += node.nov; decRev += node.dec;
                totalRevenueTax += node.tax;
            } else if (node.type === 'expense') {
                janExp += node.jan; febExp += node.feb; marExp += node.mar;
                aprExp += node.apr; mayExp += node.may; junExp += node.jun;
                julExp += node.jul; augExp += node.aug; sepExp += node.sep;
                octExp += node.oct; novExp += node.nov; decExp += node.dec;
